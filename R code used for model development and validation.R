# R code used for model development and validation
...
# same procedure applied for other continous variables including age, CD4 cell counts, CD8 cell counts, white blood cell count, platelet, haemoglobin, creatinine, triglyceride, total cholesterol, plasma glucose, aspartate transaminase, alanine aminotransferase, and total bilirubin
...
# same procedure applied for other variables including white blood cell count, platelet, creatinine, triglyceride, plasma glucose, alanine aminotransferase, and total cholesterol
...
roc.train<-timeROC(T=gzbasestack$time,
                   delta=gzbasestack$death,
                   marker=gzbasestack$lp_nomogram,
                   cause=1,
                   weighting="cox",
                   times=c(2,5,10))


roc.validation<-timeROC(T=sybasestack$time,
                        delta=sybasestack$death,
                        marker=sybasestack$lp_nomogram,
                        cause=1,
                        weighting="cox",
                        times=c(2,5,10))

par(mfrow=c(1,3))
plot(roc.train,time=2, col="#08519c", lwd=2,lty=1)
plot(roc.validation,time=2,col="#a50026",add=TRUE,lwd=2,lty=1)
legend("bottomright",c("Derivation cohort, AUC=86.8","Validation cohort, AUC=85.1"),
       col=c("#08519c","#a50026"),lwd=2)



plot(roc.train,time=5, col="#08519c", lwd=2,lty=1)
plot(roc.validation,time=5,col="#a50026",add=T,lwd=2,lty=1)
legend("bottomright",c("Derivation cohort, AUC=84.8","Validation cohort, AUC=81.5"),
       col=c("#08519c","#a50026"),lwd=2)

plot(roc.train,time=10, col="#08519c", lwd=2,lty=1)
plot(roc.validation,time=10,col="#a50026",add=T,lwd=2,lty=1)
legend("bottomright",c("Derivation cohort, AUC=77.9","Validation cohort, AUC=82.8"),
       col=c("#08519c","#a50026"),lwd=2)
